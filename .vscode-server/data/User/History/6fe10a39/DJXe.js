      ...
      tooltip: {
        ...
        callbacks: {
          title: function(tooltipItems) {
            return new Date(tooltipItems[0].parsed.x).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' });
          },
  ...
  return (
    <div className="temperatura-camaras">
      ...
      <div className="charts-grid">
        {data.map((deviceData) => {
          const datosProcesados = procesarDatos(deviceData.data);
          const chartData = {
            labels: datosProcesados.map(item => new Date(item.timestamp)),
            datasets: [
              {
                label: 'Temperatura Externa',
                data: datosProcesados.map(item => ({
