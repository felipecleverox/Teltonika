...
const TemperaturaCamaras = () => {
  ...
  const generateCSV = (camaraData) => {
    const headers = "Timestamp,Temperature\n";
    const rows = camaraData.timestamps.map((timestamp, index) => 
      `${timestamp},${camaraData.temperatures[index]}`
    ).join("\n");
    return headers + rows;
  };

  const handleDownload = (camaraData) => {
    const csvContent = generateCSV(camaraData);
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    if (link.download !== undefined) {
      const url = URL.createObjectURL(blob);
      link.setAttribute("href", url);
      link.setAttribute("download", `${camaraData.name}_temperatures.csv`);
        ...
        {data.map((camaraData) => {
          ...
          return (
            <div key={camaraData.channel_id} className="chart-container">
              ...
              <button onClick={() => handleDownload(camaraData)} className="download-button">
                Descarga de Datos
              </button>
            </div>
          );
