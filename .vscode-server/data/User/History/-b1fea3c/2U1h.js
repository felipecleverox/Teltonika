...
async function extraerDatosEYE(bleBeacons) {
  const macs = await obtenerMacsDeBaseDeDatos();
  const mac = await buscarMacEnTexto(JSON.stringify(bleBeacons), macs);
...
async function procesarEventoKTK(newData, macs) {
  const beacons = Array.isArray(newData.ble_beacons) ? newData.ble_beacons : JSON.parse(newData.ble_beacons);
...
async function procesarEventoEYE(newData, macs) {
  const bleBeacons = Array.isArray(newData.ble_beacons) ? newData.ble_beacons : JSON.parse(newData.ble_beacons);
  const eyeData = await extraerDatosEYE(bleBeacons);
