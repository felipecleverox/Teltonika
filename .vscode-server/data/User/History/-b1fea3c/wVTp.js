...
async function insertarDatosSegunIdent(ident, newData, additionalData, temp_beacon_id = null) {
  ...
  if (newData.event_enum === EVENTS.KTK) {
    ...
    // Resolvemos las promesas antes de la inserción
    const resolvedBeaconId = await Promise.resolve(temp_beacon_id);
    const resolvedEyeId = await Promise.resolve(additionalData.EYE_id);

    values = [
      ident, newData.device_id, newData.event_enum, newData.altitude, newData.latitude, newData.longitude,
      newData.timestamp, resolvedBeaconId, newData.battery_level ?? null, newData.ble_sensor_humidity_1 ?? null,
      newData.ble_sensor_magnet_status_1 ?? null, newData.ble_sensor_temperature_1 ?? null, 
      additionalData.EYE_battery_low ?? false, additionalData.EYE_humidity ?? null, resolvedEyeId, 
...
async function procesarEventoKTK(newData, macs) {
  ...
  if (highestRssiElement) {
    const ktkData = await extraerDatosKTK(highestRssiElement);
...
async function procesarEventoEYE(newData, macs) {
  ...
  if (mac) {
    const eyeData = await extraerDatosEYE(bleBeacons, mac);
    const eyeId = await obtenerIdPorMac(mac);
    await insertarDatosSegunIdent(newData.ident, newData, eyeData, eyeId);
  }
}
...
async function extraerDatosKTK(beaconElement) {
  const id = await obtenerIdPorMac(beaconElement.mac);
  return {
    KTK_ID: id,
...
async function extraerDatosEYE(bleBeacons, mac) {
