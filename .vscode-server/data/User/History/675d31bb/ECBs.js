...
function getSantiagoTime() {
  ...
  const year = now.year();
  const aprilChange = moment.tz(`${year}-04-01 00:00`, "America/Santiago").day(6);
  const septemberChange = moment.tz(`${year}-09-01 00:00`, "America/Santiago").day(6);

  if (now.isBetween(septemberChange, aprilChange)) {
    return now.utcOffset(-3 * 60);
  } else {
    return now.utcOffset(-4 * 60);
  }
}
