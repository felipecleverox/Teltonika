...
app.get('/api/temperature-dashboard-data', async (req, res) => {
  try {
    const query = `
    ...
    // Procesa los resultados para asegurarte de que external_temperature sea un número o null
...
app.get('/api/temperature-devices', async (req, res) => {
  try {
...
// Endpoint para obtener los datos de temperatura y generar el CSV
app.get('/api/temperature-data-download', async (req, res) => {
  const { deviceId, startDate, endDate } = req.query;

  try {
    const [data] = await pool.query(
      'SELECT insercion, external_temperature FROM sensor_readings_ubibot WHERE channel_id = ? AND insercion BETWEEN ? AND ?',
    ...
    const csvRows = data.map(row => {
      const formattedInsercion = moment(row.insercion).format('YYYY-MM-DD HH:mm:ss');
      return `${formattedInsercion},${row.external_temperature}`;
    });

    const csvContent = ['Insercion,External Temperature', ...csvRows].join('\n');
