...
async function insertGpsDataIntoDB(validatedData) {
  ...
  const params = columns.map(column => {
    ...
    if (value === undefined || value === null) {
      return null; // Usar null en lugar de -1 para valores no definidos
    }
    
    if (column === 'ble_beacons') {
      return JSON.stringify(value || []); // Asegurarse de que ble_beacons sea una cadena JSON
    }
